...
@Component({
  template: require('./clerkChat.html')
})
export class ClerkChat extends Vue {
  name: 'ChatWindow'
...
handleTrigger (e) {
  ...
  initWebsocketEvent () {
    ...
    this.websocket.onmessage = (event) => {
      ...
      if (result.type !== 3 && result.type !== 4 && result.type !== 5 && result.type !== 7) {
        let wavContentData = {
          wav:result.wav,
  ...
  copyText(data){
    console.log(data)
    let strText = data.roleName + data.name + "：" + data.content;
    const input = document.createElement('input');
    document.body.appendChild(input);
    input.setAttribute('value', strText);
    input.select();
  ...
  openEvi (fileAddr) {
    ...
    if (checkImg(filename)) {
      ...
      // const src = location.origin + fileAddr
      swal({
        html: `<img src="${src}" width="700px" />`,
        width: '850px',
        confirmButtonText: '关闭'
      ...
      // const src = 'https://view.officeapps.live.com/op/view.aspx?src=' + location.origin + fileAddr
      let src = ""
      if(fileAddr.indexOf("http") != -1){
        src = 'https://view.officeapps.live.com/op/view.aspx?src=' +  fileAddr
      }else{
